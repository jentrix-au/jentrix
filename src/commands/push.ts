...
import {
  ...
  type ReviewedGate,
  type ScriptResolver,
} from "../session-host/gate-classifier";
import { defaultGitRunner, inspectRepository, scopedDirtyDigest } from "../repo";
...
export interface PushFlags {
   ...
   */
  /**
   * R01 — `push report --final`: this REPORT is the explicit final deliverable
   * of the session. Writes a `final-output` header with `state: final`; the
   * server supersedes any earlier final (host-provisional or explicit) so at
   * most one current closing output exists.
   */
  final?: boolean;
  /**
   * R02 — `push report|learning --checkpoint <boundary>`: a SEMANTIC
   * checkpoint at a named boundary (compaction | handoff | intent-change |
   * task-switch | investigation-resolved | manual | none-occurred). Writes a
   * `checkpoint` header the server indexes by identity (not by title), and
   * clears a pending checkpoint request the compaction hook left.
   */
  checkpoint?: string;
  /** R02 — the current intent, one line, carried in the checkpoint header. */
  intent?: string;
  /** R02 — the next action, one line, carried in the checkpoint header. */
  next?: string;
  /**
   * INTERNAL (never a CLI option): the body was generated by the CLI itself —
   * `artifact register`'s output manifest. Lands as `source: "cli"` like the
   * `--from-cmd` capture and the session-end patches.
   */
  attested?: boolean;
  fromCmd?: string;
  /**
   * R03 — with --from-cmd: a non-zero exit is the EXPECTED result (a red gate
   * kept as evidence of a known failure). The receipt records
   * `outcome: expected-failure`; it never counts as success, and the CLI still
...
// ---------------------------------------------------------------------------
// R03 — the STRUCTURED verification receipt (prds/opencode-pi-plugins-prd.md
// §4.1). The audit's probe showed an attested LOG named `echo pnpm test`
// satisfying a "tests passed" claim, and the 2026-09-12 review showed
// `node -e "console.log('test')"` and `… || node -e "process.exit(0)"` doing
// the same under the first fix. The classification now lives in ONE pure
// module shared with the server (`session-host/gate-classifier.ts`): a gate
// is a reviewed wrapper, a package script resolved against the package.json
// it ran in (name → family, body recorded), or a known runner binary — never
// words of the line, never a composed line other than `&&`. The receipt
// carries what a verifier needs to bind the claim to that definition: the
// script bodies and their digests, the reviewed file's digest, where (cwd,
// repository, revision, dirty-tree digest), when, exit code and outcome.
// Legacy `$ command` opener first, so an older server still reads line one.
// ---------------------------------------------------------------------------
...
/** Bound on the script body text a receipt embeds (the digest covers all of it). */
export const MAX_RECEIPT_SCRIPT_BODY_CHARS = 2000;

/**
 * The package.json the command ran against: the nearest one at or above
 * `cwd` (how pnpm/npm/yarn resolve a script). Returns the {@link ScriptResolver}
 * the classifier consumes — `null` for an unknown name, and a resolver that
 * refuses everything when no package.json exists (the command could not have
 * run a script there either).
 */
export function packageScriptResolver(
  cwd: string,
  ...
  for (let depth = 0; depth < 32; depth += 1) {
    try {
      const pkg = JSON.parse(readFile(join(dir, "package.json"))) as { scripts?: Record<string, unknown> };
      const scripts = pkg.scripts && typeof pkg.scripts === "object" ? pkg.scripts : {};
      return (_manager, name) => (typeof scripts[name] === "string" ? { body: scripts[name] as string } : null);
    } catch {
...
/**
 * Reviewed wrappers: a committed, code-reviewed list that lets a repository
 * name its own gate commands (a script under `scripts/`, a `make` target) so
 * they count as gates without widening the executable allowlist. The file's
 * digest rides the receipt so a verifier can tell which definition applied.
 */
export function readReviewedGates(
  root: string | null,
  readFile: (path: string) => string = (path) => readFileSync(path, "utf8"),
): { gates: ReviewedGate[]; digest: string | null } {
  if (!root) return { gates: [], digest: null };
  try {
    const text = readFile(join(root, ".jentrix", "gates.json"));
    const parsed = JSON.parse(text) as { gates?: unknown };
    const gates = (Array.isArray(parsed.gates) ? parsed.gates : [])
      .filter(
        (g): g is ReviewedGate =>
          typeof g === "object" && g !== null &&
          typeof (g as ReviewedGate).id === "string" &&
          typeof (g as ReviewedGate).command === "string" &&
          Array.isArray((g as ReviewedGate).families),
      )
      .map((g) => ({ id: g.id, command: g.command.trim(), families: g.families.filter((f): f is GateFamily => (GATE_FAMILIES as readonly string[]).includes(f)) }));
    return { gates, digest: createHash("sha256").update(text, "utf8").digest("hex") };
  } catch {
    return { gates: [], digest: null };
  }
}

// R03 — `scopedDirtyDigest` lives in ../repo (the session host binds the
// close to the tree with it too); re-exported here for the existing callers.
export { scopedDirtyDigest } from "../repo";
...
export interface VerificationReceiptInput {
  ...
  output: string;
  cwd: string;
  repo: { ownerName: string; root?: string | null; revision: string | null; dirtyDigest: string | null } | null;
  startedAt: string;
  endedAt: string;
  /** The operator declared this failure EXPECTED (a red gate as evidence). */
  expectedFailure?: boolean;
  reviewed?: { gates: ReviewedGate[]; digest: string | null };
  /** F01 — resolves `<manager> run <name>` against the package.json the command ran in. */
  resolveScript?: ScriptResolver;
}

/**
 * The attested LOG body (R03): legacy opener, the structured receipt, then
 * the bounded output tail. `fromCmdLogBody` stays the opener+tail builder the
 * older tests pin; this wraps it.
 */
export function buildVerificationReceipt(input: VerificationReceiptInput): string {
  // The CLI never classifies a package script by NAME alone: with no
  // resolver there is no package.json, and the command could not have run a
  // script there either.
  const gate = classifyGateCommand(input.command, {
    reviewed: input.reviewed?.gates ?? [],
    resolveScript: input.resolveScript ?? (() => null),
  });
  const outputBytes = Buffer.byteLength(input.output, "utf8");
  const retained = Math.min(outputBytes, MAX_FROM_CMD_OUTPUT_BYTES);
  const outcome =
    input.exitCode === 0 ? "success" : input.expectedFailure ? "expected-failure" : "failure";
  const header = renderSemanticHeader({
    kind: "verification-receipt",
    runner: `@jentrix/cli ${CLI_VERSION}`,
    command: input.command,
    gateExecutable: gate.executable,
    gateAllowlisted: gate.allowlisted,
    gateFamilies: gate.families,
    gateId: gate.gateId,
    gateSource: gate.source,
    gatesDigest: input.reviewed?.digest ?? null,
    // F01 — the script definitions the verdict rests on: body digest over
    // the whole body, text bounded; the server re-classifies the body and a
    // verifier checks the digest against package.json at `revision`.
    gateScripts: gate.scripts.map((script) => ({
      manager: script.manager,
      name: script.name,
      ...(script.body !== null
        ? {
            digest: createHash("sha256").update(script.body, "utf8").digest("hex"),
            body: script.body.slice(0, MAX_RECEIPT_SCRIPT_BODY_CHARS),
            ...(script.body.length > MAX_RECEIPT_SCRIPT_BODY_CHARS ? { bodyTruncated: true } : {}),
          }
        : {}),
    })),
    ...(gate.reason ? { gateReason: gate.reason } : {}),
    cwd: input.cwd,
    // F02a — the checkout root the digest describes; the floor binds only
    // when `cwd` lies inside it (the classifier already refuses `cd`).
    repoRoot: input.repo?.root ?? null,
    repo: input.repo?.ownerName ?? null,
    revision: input.repo?.revision ?? null,
    dirtyDigest: input.repo?.dirtyDigest ?? null,
    ...
    outputComplete: outputBytes <= MAX_FROM_CMD_OUTPUT_BYTES,
  });
  const legacy = fromCmdLogBody(input.command, input.exitCode, input.output);
  const opener = `$ ${input.command}\nexit code: ${input.exitCode}\n\n`;
  const tail = legacy.startsWith(opener) ? legacy.slice(opener.length) : legacy;
...
export async function runPush(
  ...
  try {
    ...
    // R01/R02 flag hygiene: a header belongs to the kind it describes.
    if (flags.final && kind !== "report") {
      throw new UsageError("--final marks the explicit final deliverable — it only applies to `jentrix push report --final`");
    }
    if (flags.checkpoint !== undefined) {
      if (kind !== "report" && kind !== "learning") {
        throw new UsageError("--checkpoint applies to `jentrix push report|learning --checkpoint <boundary>`");
      }
      if (!(CHECKPOINT_BOUNDARIES as readonly string[]).includes(flags.checkpoint)) {
        throw new UsageError(`--checkpoint needs one of: ${CHECKPOINT_BOUNDARIES.join(" | ")}`);
      }
      if (flags.final) throw new UsageError("--final and --checkpoint describe different artifacts — push them separately");
    }
    if ((flags.intent !== undefined || flags.next !== undefined) && flags.checkpoint === undefined) {
      throw new UsageError("--intent/--next ride a checkpoint — pass --checkpoint <boundary>");
    }
    ...
    if (flags.fromCmd !== undefined) {
      ...
      // R03: the receipt names WHERE and AGAINST WHAT the gate ran. The
      // checkout is read AFTER the command so the dirty digest describes the
      // tree the gate actually tested (a gate that writes fixtures still
      // digests what it left). Unreadable checkout ⇒ nulls, never guesses.
      let repo: VerificationReceiptInput["repo"] = null;
      ...
      try {
        ...
        if (inspection) {
          ...
          repo = {
            ownerName: inspection.repoOwnerName,
            root: inspection.root,
            revision: inspection.head ?? null,
            dirtyDigest: await scopedDirtyDigest(deps.git ?? defaultGitRunner, inspection.root),
          };
        }
      } catch {
        repo = null;
      }
      const reviewed = readReviewedGates(root);
      const resolveScript = packageScriptResolver(deps.cwd());
      raw = buildVerificationReceipt({
        command,
        exitCode: ran.code,
        output: ran.output,
        cwd: deps.cwd(),
        ...
        startedAt,
        endedAt,
        expectedFailure: flags.expectedFailure === true,
        reviewed,
        resolveScript,
      });
      const gate = classifyGateCommand(command, { reviewed: reviewed.gates, resolveScript });
      if (!gate.allowlisted) {
        deps.writeErr(
          `note: ${gate.reason ?? `\`${gate.executable ?? command}\` is not a gate runner`} — this LOG is attested output, but it will not cover a typecheck/lint/test/e2e claim (gates are package scripts, known runners, or reviewed wrappers in .jentrix/gates.json)`,
        );
      }
    } else {
      raw = file
    ...
    const withBasis = flags.basis?.length
      ...
      : flags.noBasis !== undefined
        ...
        : raw;
    // R01/R02 — the semantic header, rendered from the same values the
    // server will parse back (R08: one source, machine and human).
    const headed = flags.final
      ...
      : flags.checkpoint !== undefined
        ? `${renderSemanticHeader({
            kind: "checkpoint",
            boundary: flags.checkpoint,
            ...(flags.intent ? { currentIntent: flags.intent } : {}),
            ...(flags.next ? { nextAction: flags.next } : {}),
            noneOccurred: flags.checkpoint === "none-occurred",
            declaredAt: new Date().toISOString(),
          })}\n\n${withBasis}`
        : withBasis;

    // Local redaction BEFORE anything leaves the process (PRD §6).
    const body = createSessionRedactor({ env: deps.env }).text(headed);
      ...
      (deps.fetchImpl ?? fetch)(
        new URL(to.path, stacksBaseUrlOf(credentials.url)),
        {
          ...
          body: JSON.stringify({
            ...
            ...(commandExit !== null || flags.attested === true ? { attested: true } : {}),
            body,
          }),
        },
      );
    ...
    deps.writeOut(
      flags.json
        ...
        : `Pushed ${kind} → artifact ${payload.artifactId}${payload.deduped ? " (already stored — identical content)" : ""} · ${target.label}${flags.final ? " · explicit FINAL output" : ""}${flags.checkpoint ? ` · checkpoint (${flags.checkpoint})` : ""}`,
    );
    // R02: a semantic checkpoint answers the hook's request; record it and
    // clear the request so `status`/`end` stop asking.
    if (flags.checkpoint !== undefined && correlatedSessionId && payload.artifactId) {
      try {
        writeFileSync(
          join(deps.spoolRoot, correlatedSessionId, "checkpoint.json"),
          JSON.stringify({ artifactId: payload.artifactId, boundary: flags.checkpoint, at: new Date().toISOString() }),
          { mode: 0o600 },
        );
        unlinkSync(checkpointRequestPath(deps.spoolRoot, correlatedSessionId));
      } catch {
        // absent request or unwritable ledger — the artifact is stored either way
      }
    }
...
export function registerPushCommand(
  ...
  program
    ...
    .option(
      "--final",
      "report pushes only: this REPORT is the explicit final deliverable — supersedes any earlier final output; at most one current closing output per session",
    )
    .option(
      "--checkpoint <boundary>",
      `report|learning pushes only: a semantic checkpoint at a boundary (${CHECKPOINT_BOUNDARIES.join(" | ")}); clears a pending checkpoint request`,
    )
    .option("--intent <line>", "with --checkpoint: the current intent, one line")
    .option("--next <line>", "with --checkpoint: the next action, one line")
    .option(
      "--from-cmd <command>",
      'log pushes only: run the command locally, capture exit code + a 64 KB tail-biased output tail, and push an ATTESTED LOG opening with both and a structured verification receipt (runner, gate, cwd, revision, dirty digest, timing, outcome) — then exit with the command\'s own code ("tests green" claims carry evidence)',
    )
    .option(
      "--expected-failure",
      "with --from-cmd: a non-zero exit is the expected result — recorded as outcome expected-failure, never as success",
    )
    .option("--json", "stable JSON output")
