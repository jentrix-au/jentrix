---
description: Align this Pi session to a Jentrix work item (task + accountable owner) with a server-confirmed snapshot
---
...
1. If the operator named a work item (e.g. `/jentrix-align JEN-42`), run with
   ...
   ```
   jentrix session align --provider pi --task <that value> --json
   ```

   For explicit session-level work ("no task"), run
   `jentrix session align --provider pi --session-level --json` instead — but only when the
