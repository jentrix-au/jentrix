---
description: Connect this OpenCode session to Jentrix (session anchor + capture in the folder's workspace)
---
...
```
jentrix session connect --provider opencode
```
...
- Connect is IDENTITY only. Anchoring work is `/jentrix-align`
  (`jentrix session align --provider opencode --task <id-or-key>`), and a Project is an optional
