...
| `marketplace <provider> User-managed: jentrix → <path> was not written by this CLI` | Someone registered `jentrix` from a checkout or a fork. The installer refuses to repoint it, by design. Fix, with the customer's consent: `<provider> plugin marketplace remove jentrix` then `jentrix plugin install <provider>` — or leave it if it is their fork ([forks](../forks.md)) |
| `marketplace <provider> Unknown: … exited <code>` | The provider's own listing failed. Run `<provider> plugin marketplace list --json` by hand and read its error; a dangling row of ours is removed and retried by the installer, a foreign broken row is relayed untouched |
