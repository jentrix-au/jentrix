...
export const CORPUS: Array<{
  ...
  // candidate-6 review (2026-09-13): a comment ends at ITS line — the newline after it is a second command; any `$` outside single quotes is an expansion the receipt cannot see through
  { command: "node --test fail.test.mjs # comment\ntrue", allowlisted: false, reason: "newline" },
  { command: "# note\npnpm test", scripts: { test: "vitest run" }, allowlisted: false, reason: "newline" },
  { command: "pnpm test # comment", scripts: { test: "vitest run" }, allowlisted: true, families: ["test"], source: "script" },
  { command: "node --test \"$C6_REVIEW_FLAG\"", allowlisted: false, reason: "variable expansion" },
  { command: "node --test \"$C6_REVIEW_TEST\"", allowlisted: false, reason: "variable expansion" },
  { command: "node --test $C6_REVIEW_TEST", allowlisted: false, reason: "variable expansion" },
  { command: "node --test \"${C6_REVIEW_TEST}\"", allowlisted: false, reason: "variable expansion" },
  { command: "X=--help node --test $X", allowlisted: false, reason: "variable expansion" },
  { command: "FLAG=$Y node --test pass.test.mjs", allowlisted: false, reason: "variable expansion" },
  { command: "pnpm test >$OUT", scripts: { test: "vitest run" }, allowlisted: false, reason: "variable expansion" },
  { command: "pnpm test -- --grep 'end$'", scripts: { test: "vitest run" }, allowlisted: true, families: ["test"], source: "script" },
  { command: "pnpm test \"\\$literal\"", scripts: { test: "vitest run" }, allowlisted: true, families: ["test"], source: "script" },
  { command: "X=--help node --test pass.test.mjs", allowlisted: true, families: ["test"], source: "runner" },
