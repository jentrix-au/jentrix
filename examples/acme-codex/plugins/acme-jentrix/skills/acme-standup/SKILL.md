---
name: acme-standup
description: Acme standup — what moved on our Jentrix boards in the last day, grouped by column, over the Jentrix CLI.
---

# Acme standup

Produce Acme's standup summary from Jentrix using ONLY the `jentrix` CLI.
Never ask for or print a token; the CLI reads its credential from
`jentrix login` or `STACKS_TOKEN` itself.
...
verbatim and stop — `RATE_LIMITED` carries `retryAfterSeconds` (the CLI
already waits up to 60 s by itself); `FORBIDDEN` means the token lacks the
`read` scope for that workspace. Do not retry a `FORBIDDEN` or `NOT_FOUND`
call with different ids you guessed.
